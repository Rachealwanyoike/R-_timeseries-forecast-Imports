...
naive.roll <- ts(export$NX[fixed.nTrain:(fixed.nTrain + fixed.nValid - 1)], start = c(1990, fixed.nTrain + 1), 
                 end = c(1990, fixed.nTrain + fixed.nValid), freq = 12)

#predictive measures
accuracy(naive.fixed$mean, valid.ts)

#plot residuals
plot(naive.fixed$residuals, ylim = c(-1500, 1000),  ylab = "Residuals", xlab = "Time", bty = "l", xaxt = "n", 
     xlim = c(1990,2027.25), main = "")
axis(1, at = seq(1990, 2027, 1), labels = format(seq(1990, 2027, 1)))
...
plot(Net_export.lm.pred$residuals, ylim = c(-1500, 1000),  ylab = "Residuals", xlab = "Time", bty = "l", xaxt = "n", 
     xlim = c(1990,2027.25), main = "")
axis(1, at = seq(1990, 2027, 1), labels = format(seq(1990, 2027, 1)))
